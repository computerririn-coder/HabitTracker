  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4">
        {/* Icon */}
        <div
          className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border bg-linear-to-br ${isUnlocked ? achievement.bgGradient : 'from-slate-700/50 to-slate-800/50'} ${isUnlocked ? achievement.borderColor : 'border-slate-600/30'}`}
        >
          <Icon
            className={`h-7 w-7 ${isUnlocked ? achievement.iconColor : 'text-slate-500'}`}
          />
        </div>
        ...
        <div className="min-w-0 flex-1">
          <h3
            className={`text-lg font-semibold ${isUnlocked ? 'text-cyan-100' : 'text-slate-400'}`}
          >
            {achievement.name}
          </h3>
          <p
            className={`mt-1 text-sm ${isUnlocked ? 'text-cyan-400/70' : 'text-slate-500'}`}
          >
