...
const useStore = create<AchievementStore>((set) => ({
  achievements: (() => {
    ...
    if (saved) {
      ...
      return parsed.map(
        (achievement: Omit<Achievement, 'icon'>): Achievement => ({
          ...achievement,
          icon: Target,
        })
      );
  ...
  unlock: (id: number) =>
    set((state: AchievementStore) => {
      const newAchievements: Achievement[] = state.achievements.map(
      ...
      const toSave: Omit<Achievement, 'icon'>[] = newAchievements.map(
        ({ icon, ...rest }) => rest
      );
...
const useTotalTabCreatedCount = create<TotalTabCreatedCountStore>((set) => ({
  ...
  incrementTabCount: () =>
    set((state: TotalTabCreatedCountStore) => {
...
const useCompletedTasksCount = create<CompletedTasksCountStore>((set) => ({
  ...
  incrementCompletedTasksCount: () =>
    set((state: CompletedTasksCountStore) => ({
...
const useTotalTaskCompletion = create<TotalTaskCompletionStore>((set) => ({
  totalTaskCompletion: parseInt(
    localStorage.getItem('totalTaskCompletion') || '0'
  ),
