import { useState, useContext } from 'react';
import { useForm } from 'react-hook-form';
import { TabNumberContext } from './TasksBar';
import {
  useAchievementCount,
  useComponentVisibility,
  useStore,
  useTotalTabCreatedCount,
  type FormData,
} from './store';

function AddNewTab() {
  //From store(zustand)
  const unlock = useStore((state) => state.unlock);
  const totalTabCreatedCount = useTotalTabCreatedCount(
    (state) => state.totalTabCreatedCount
  );
  const incrementTabCount = useTotalTabCreatedCount(
    (state) => state.incrementTabCount
  );
  const setComponentVisibility = useComponentVisibility(
    (state) => state.setComponentVisibility
  );
  const incrementAchievementCount = useAchievementCount(
    (state) => state.incrementAchievementCount
  );

  const { tabs, setTabs, setTabCount } = useContext(TabNumberContext)!;
  const [existingKeyMsg, setExistingKeyMsg] = useState(false);
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm({
    defaultValues: {
      taskName: 'Default',
      current: 0,
      max: 10,
      hotKey: '1',
      hotKey2: 'Q',
      totalCompletionColor: false,
    },
  });

  const onSubmit = (data: FormData) => {
    const newHotKey = `${data.hotKey.toUpperCase()}+${data.hotKey2.toUpperCase()}`;
    const existingHotKeys = tabs.map((e) => e.hotKey);

    if (existingHotKeys.includes(newHotKey)) {
      setExistingKeyMsg(true);
      return;
    } else {
      setExistingKeyMsg(false);
    }

    if (totalTabCreatedCount === 0) {
      unlock(1);
      window.alert('Achievement Unlock');
      incrementTabCount();
      incrementAchievementCount();
    }

    const newTab = {
      current: Number(data.current),
      max: Number(data.max),
      name: data.taskName,
      hotKey: newHotKey,
      dateHistory: [],
      completionCount: 0,
      totalCompletionColor: false,
    };

    setTabs((prev) =>
      [...prev, newTab].map((tab, index) => ({ ...tab, id: index }))
    );
    setTabCount((e) => e + 1);

    setComponentVisibility((prev) => ({
      ...prev,
      addNewTab: false,
    }));
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm  2xl:scale-160">
      <div className="  mx-4 w-full max-w-md rounded-2xl border border-cyan-500/30 bg-linear-to-br from-slate-800 via-slate-900 to-slate-950 shadow-2xl shadow-cyan-500/20">
        {/* Header */}
        <div className="relative border-b border-cyan-500/20 p-6">
          <h2 className="bg-linear-to-r from-cyan-400 to-blue-400 bg-clip-text text-2xl font-bold text-transparent">
            Create New Task
          </h2>

          <button
            className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full border border-red-500/30 bg-red-500/20 text-red-400 transition-all duration-200 hover:scale-110 hover:bg-red-500/30 hover:text-red-300"
            onClick={() =>
              setComponentVisibility((prev) => ({
                ...prev,
                addNewTab: false,
              }))
            }
          >
            ×
          </button>
        </div>
        {/* Form */}
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 p-6">
          {/* Task Name */}
          <div className="space-y-2">
            <label
              htmlFor="taskName"
              className="block text-sm font-medium text-cyan-300"
            >
              Task Name
            </label>
            <input
              id="taskName"
              {...register('taskName', {
                required: 'This field is required',
                minLength: {
                  value: 1,
                  message: 'This field is required',
                },
                validate: {
                  notEmpty: (value) =>
                    value.trim().length > 0 || 'This field is required',
                },
              })}
              placeholder="Enter task name"
              className={`w-full rounded-lg border px-4 py-3 bg-slate-900/50 text-cyan-50 placeholder-cyan-600/50 transition-all focus:outline-none focus:ring-2 ${
                errors.taskName
                  ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                  : 'border-cyan-700/50 focus:border-cyan-500 focus:ring-cyan-500/30'
              }`}
            />
            {errors.taskName && (
              <p className="text-sm text-red-400">{errors.taskName.message}</p>
            )}
          </div>

          {/* Current and Max */}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label
                htmlFor="current"
                className="block text-sm font-medium text-cyan-300"
              >
                Current Progress
              </label>
              <input
                id="current"
                {...register('current', {
                  valueAsNumber: true,
                  required: 'This field is required',
                  min: {
                    value: 0,
                    message: '',
                  },
                })}
                type="number"
                placeholder="0"
                className={`w-full rounded-lg border px-4 py-3 bg-slate-900/50 text-cyan-50 placeholder-cyan-600/50 transition-all focus:outline-none focus:ring-2 ${
                  errors.current
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                    : 'border-cyan-700/50 focus:border-cyan-500 focus:ring-cyan-500/30'
                }`}
              />
              {errors.current && (
                <p className="text-sm text-red-400">{errors.current.message}</p>
              )}
            </div>

            <div className="space-y-2">
              <label
                htmlFor="max"
                className="block text-sm font-medium text-cyan-300"
              >
                Daily Goal
              </label>
              <input
                id="max"
                {...register('max', {
                  valueAsNumber: true,
                  required: 'This field is required',
                  min: {
                    value: 1,
                    message: 'Must Be Equal Or Greater Than 1',
                  },
                })}
                type="number"
                placeholder="10"
                className={`w-full rounded-lg border px-4 py-3 bg-slate-900/50 text-cyan-50 placeholder-cyan-600/50 transition-all focus:outline-none focus:ring-2 ${
                  errors.max
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                    : 'border-cyan-700/50 focus:border-cyan-500 focus:ring-cyan-500/30'
                }`}
              />
              {errors.max && (
                <p className="text-sm text-red-400">{errors.max.message}</p>
              )}
            </div>
          </div>

          {/* Hotkey */}
          <div className="space-y-2">
            <label className="block text-center text-sm font-medium text-cyan-300">
              Hotkey Combination
            </label>
            <div className="flex items-center gap-5">
              <input
                {...register('hotKey', {
                  required: 'This field is required',
                  minLength: {
                    value: 1,
                    message: 'This field is required',
                  },
                  maxLength: {
                    value: 1,
                    message: 'Maximun 1 Characters',
                  },
                  validate: {
                    notEmpty: (value) =>
                      value.trim().length > 0 || 'This field is required',
                  },
                })}
                placeholder="F2"
                className={`w-10 flex-1 rounded-lg border px-4 py-3 bg-slate-900/50 text-center font-mono text-cyan-50 placeholder-cyan-600/50 transition-all focus:outline-none focus:ring-2 ${
                  errors.hotKey
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                    : 'border-cyan-700/50 focus:border-cyan-500 focus:ring-cyan-500/30'
                }`}
              />

              <span className="text-lg font-bold text-cyan-400">+</span>

              <input
                {...register('hotKey2', {
                  required: 'This field is required',
                  minLength: {
                    value: 1,
                    message: 'This field is required',
                  },
                  maxLength: {
                    value: 1,
                    message: 'Maximun 1 Characters',
                  },
                  validate: {
                    notEmpty: (value) =>
                      value.trim().length > 0 || 'This field is required',
                  },
                })}
                placeholder="K"
                className={`w-10 flex-1 rounded-lg border px-4 py-3 bg-slate-900/50 text-center font-mono text-cyan-50 placeholder-cyan-600/50 transition-all focus:outline-none focus:ring-2 ${
                  errors.hotKey2
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                    : 'border-cyan-700/50 focus:border-cyan-500 focus:ring-cyan-500/30'
                }`}
              />
            </div>
            {(errors.hotKey || errors.hotKey2) && (
              <p className="text-center text-sm text-red-400">
                {errors.hotKey?.message || errors.hotKey2?.message}
              </p>
            )}
          </div>

          {/* Submit Button */}
          <button
            type="submit"
            className="mt-6 w-full transform rounded-lg bg-linear-to-r from-cyan-600 to-blue-600 px-6 py-3.5 font-semibold text-white shadow-lg shadow-cyan-500/30 transition-all duration-300 hover:scale-[1.02] hover:from-cyan-500 hover:to-blue-500 hover:shadow-cyan-500/50 active:scale-[0.98]"
          >
            Create Tracker
          </button>
          <p className="text-center text-sm text-red-400">
            {existingKeyMsg
              ? 'Hotkey Already Exists, Please Use A Different Hotkey'
              : ''}
          </p>
        </form>
      </div>
    </div>
  );
}

export default AddNewTab;
