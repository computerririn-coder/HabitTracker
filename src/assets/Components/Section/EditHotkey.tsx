...
function EditHotkey() {
  const { currentTab, tabs, setTabs, setComponentVisibility } =
  ...
  const onSubmit = (data: EditHotKeyFormData) => {
    ...
    const hasDuplicate = hotKeysDuplicateCheck.some(
      (item, index) => hotKeysDuplicateCheck.indexOf(item) !== index
    );
    console.log(newHotKey);
    if (!hasDuplicate) {
      setTabs((prev) =>
        prev.map((tab, index) =>
          index === currentTab ? { ...tab, hotKey: newHotKey } : tab
        )
      );
      setErrorMsg(false);
  ...
  return (
    <section className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm xl:scale-150">
      <div className="mx-4 w-full max-w-md rounded-2xl border border-cyan-500/30 bg-linear-to-br from-slate-800 via-slate-900 to-slate-950 shadow-2xl shadow-cyan-500/20">
        ...
        <div className="relative border-b border-cyan-500/20 p-6">
          ...
          <button
            ...
            onClick={() =>
              setComponentVisibility((prev) => ({ ...prev, editHotKey: false }))
            }
          >
            ×
          </button>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 p-6">
          ...
          {errorMsg && (
            <p className="text-center text-red-700 text-sm">
              {errorMsg
                ? 'Duplicate HotKey Detected Please Choose A Different HotKey'
                : ''}
            </p>
          )}
          <button
            type="submit"
            className=" w-full transform rounded-lg bg-linear-to-r from-cyan-600 to-blue-600 px-6 py-3.5 font-semibold text-white shadow-lg shadow-cyan-500/30 transition-all duration-300 hover:scale-[1.02] hover:from-cyan-500 hover:to-blue-500 hover:shadow-cyan-500/50 active:scale-[0.98]"
          >
            Save Changes
          </button>
        </form>
